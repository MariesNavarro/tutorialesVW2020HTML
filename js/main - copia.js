...
function popPreview(c,n, t, nm, f,u){
  var num = n;
  var name = nm;
  ...
  if(c === "open"){
    ...
    $("#btnTerminar").attr("onclick","popPreview('close', 0, this, '"+name+"', '"+f+"', '"+u+"')");
    setTimeout(function(){
      wr.style.opacity = "1";
    },600);
  } else { // close
  ...
  setTimeout(function(){
    t.setAttribute("onclick", "popPreview('"+c+"',"+n+", this, '"+name+"', '"+f+"', '"+f+"')");
  },1000);
}
...
/******** regsitrar Ticket *******/
function update_seccion(user,sec) {
		  var dataString ='&acc=1&id='+user+'&sec='+sec;
		  console.log('update_seccion:'+dataString);
		  $.ajax({
		         type   : 'POST',
		         url    : 'respuesta.php',
		         data   :  dataString,
               ...
               if (data=='Ok') {
                 $("#"+sec).removeClass("seenTut");
                 $("#"+sec).addClass("noseenTut");
               }
		         }
		  });
}
